...
class ExpenseGenerator {
  constructor(expenseData, expenseConfig) {
    this.expenseData = expenseData
    if(Number.isInteger(expenseConfig)) this.numExpenses = expenseConfig
    if(expenseConfig instanceof Object) this.expenseObj = expenseConfig
    this.expenseDataLength = this.expenseData.length
  }

  generateExpenses() {
    const expenses = []
    for(let date in this.expenseObj) {
      let numExpenses = this.expenseObj[date]
      while(numExpenses !== 0) {
        const expenseIndex = numExpenses
        const expensesData = this.expenseData[expenseIndex]
        const expense = new Expense(expensesData) 
        this.addDateToExpense(date, expense)
  ...
  addDateToExpense(date, expense) {
    switch (date) {
      ...
      case 'one_month_ago':
        expense['date'] = DateGenerator.getOneMonthAgoDate()
        break;
